...
class TrialEnv(object):
    ...
    def rover_pos_callback(self, msg: PointStamped):
        ...
        rover = np.array([self.rover_pos.point.x, self.rover_pos.point.y])
    ...
    def wait_for_service(self, service):
        """wait for mavros and gazebo service"""
        ...
        while not rospy.is_shutdown():
            for sub_topic in self.sub_topics_ready.keys():
                rospy.loginfo_throttle(
                    10, "%s ready: %d", sub_topic, self.sub_topics_ready[sub_topic]
                )
    ...
    def wait_for_landed_state(self, desired_landed_state: int):
        ...
        while not rospy.is_shutdown():
            if self.extended_state.landed_state == desired_landed_state:
                ...
                rospy.loginfo_throttle(
                    10,
                    "waiting for landed state | state: {0}".format(
                        mavutil.mavlink.enums["MAV_LANDED_STATE"][
    ...
    def wait_for_mode(self, desired_mode: str):
        ...
        while not rospy.is_shutdown():
            ...
            if self.state.mode == desired_mode:
                ...
                rospy.loginfo_throttle(
                    10,
                    "waiting for mode {0} | state: {1}".format(
    ...
    def send_pos(self):
        ...
        while not rospy.is_shutdown():
            ...
            rospy.logdebug(
                "sending offboard {0:.2f} {1:.2f} {2:.2f}".format(
