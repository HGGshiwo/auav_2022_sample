...
class A3C(nn.Module):
    ...
    def __init__(
        ...
        critic_atten_layers = [
            ...
            nn.MultiheadAttention(n_features, 4, 0.2),
        ]
        ...
        self.critic_atten = nn.Sequential(*critic_atten_layers).to(self.device)
        self.actor_atten = nn.Sequential(*actor_atten_layers).to(self.device)
    ...
    def forward(self, x: np.ndarray) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        attn_output1, attn_output_weight = self.critic_atten(
            feature, self.state_que, self.state_que
        )
        x = attn_output1 + feature  # 残差连接
        state_values = self.critic(x)

        attn_output2, attn_output_weight = self.actor_atten(
            feature, self.state_que, self.state_que
        )
        x = attn_output2 + feature  # 残差连接
